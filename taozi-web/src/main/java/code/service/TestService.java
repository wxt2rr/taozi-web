...
public class TestService {
    ...
    public GlobalResponse test(String name) {
        InputStream license = null;
        try {
            license = new FileInputStream(new File("/test/license.xml"));
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        if(license == null){
            return GlobalResponse.createByError(-1,"license");
        }
        try {
            new License().setLicense(license);
        } catch (Exception e) {
            return GlobalResponse.createByError(-3, Arrays.toString(e.getStackTrace()));
        }
        Map<String,String> map = new HashMap<String,String>();
        map.put("${enterpriseName}","王小涛");
        map.put("${enterprisePhone}","13161584722");
        map.put("${enterpriseAddress}","北京");
        map.put("${userId}",String.valueOf(1));
        map.put("${y}","2019");
        map.put("${m}","02");
        map.put("${d}","23");

        String srcPath = "/test/source.pdf";
        String targetPath = "/test/target.pdf";
        String s = "${enterpriseName}";
        String s1 = map.get(s);
        try{
            Document pdfDoc = new Document(srcPath);
                ...
                for (TextFragment textFragment :(Iterable<TextFragment>) textFragmentAbsorber.getTextFragments()) {
                    textFragment.setText(s1);
                    Color color = Color.fromCmyk(0, 0, 0,0.95);
                    textFragment.getTextState().setForegroundColor(color);
                }

            pdfDoc.save(targetPath);
        }catch (Exception e){
            return GlobalResponse.createByError(-4,s+","+s1+Arrays.toString(e.getStackTrace()));
        }
        return GlobalResponse.createBySuccess();
    }
}
